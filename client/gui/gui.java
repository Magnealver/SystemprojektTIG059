...
public class gui extends JFrame {
	...
	SQL sqlobject = new SQL(); //För att kunna kontakta databasen genom SQL-frågor
	
 	//Alla fönster som används
	...
	AllCustomersWin w6 = new AllCustomersWin();
 	...
 	private class AddSafariWin extends JFrame{ //Fönster för att skapa safarin
	...
	private AddSafariWin() {
 	...
 	private class UpdateSafariWin extends JFrame{ //Fönster för att uppdatera safarin
	...
	private AddOccasionWin() {
 	...
 	private class SafariBookingWin extends JFrame{ //Fönster för att skapa safaribokningar
		...
		JButton wb9 = new JButton("Inform Guide");
		...
		ActionListener wbl = new ActionListener() {
	    	public void actionPerformed(ActionEvent e) { 
				if (e.getSource() == wb1) {
        			...
        			switch (choice) {
        				case 0: //Om man väljer att boka kommer nytt bokningsID att skapas för bokningen och därefter kommer en bokning att göras med den data som matats in
        				try {
        					if (isParsable(custIDt.getText()) && sqlobject.checkCustomerID(Integer.parseInt(custIDt.getText()))) { //Om kundID anges och existerar, skapa ny bokning åt den kunden.
        						...
        						&& (phonenmbrt.getText().matches("\\d*")) && (!emailt.getText().isEmpty())) { //Om kundID inte redan finns skapas en ny kund med den data som matats in. 
        						...
        						switch (pay) {
        							case 0:
        								JOptionPane.showMessageDialog(payment, "Customer created. Payment received. Booking confirmed. BookingID: " //Bekräftelse och simulering av betalning
				...
				} else if ((e.getSource() == wb9) && (cb2.getSelectedItem() != null)) {
					JOptionPane.showMessageDialog(null, "Information regarding occasion '"+cb.getSelectedItem().toString()+", "
						+cb2.getSelectedItem().toString()+"' sent to guide!", "Information", JOptionPane.INFORMATION_MESSAGE);
				} else if ((e.getSource() == wb9) && (cb2.getSelectedItem() == null)) {
					JOptionPane.showMessageDialog(null, "Choose a date to inform guide of!", "Error", JOptionPane.ERROR_MESSAGE);
				}
		    }
		};
	...
	private SafariBookingWin() { 
		...
		wp5.add(wb3); wp5.add(wb6); wp5.add(wb9); wp5.add(wb8);
		c.gridy = 2;
		c.fill = GridBagConstraints.BOTH;
		c.gridwidth = GridBagConstraints.REMAINDER;
		add(wp5, c);
	...
	private class AllCustomersWin extends JFrame{ //Fönster för att söka efter kunder
		...
		ActionListener wbl = new ActionListener() {
	    	public void actionPerformed(ActionEvent e) {
				if (e.getSource() == wb1) {
					...
					if ((wf1.getText().matches("\\d*")) && (!wf1.getText().isEmpty()) //Söker på kundID 
        				...
        				int result = JOptionPane.showOptionDialog(null, scrollPane, "Information", JOptionPane.DEFAULT_OPTION,JOptionPane.PLAIN_MESSAGE, null, options, options[0]);
        				switch (result) {
        					case 0:			//Presenterar data för vald kund
        					...
        					case 1:
        					break;
        					case 2: //Visar alla bokningar som tillhör vald kund.
        					cb4.removeAllItems();
            		...
            		} else if ((wf2.getText().matches("\\D*")) && (wf3.getText().matches("\\D*")) && (!wf2.getText().isEmpty()) && (!wf3.getText().isEmpty())) { //Söker efter kund med namn
        				...
        				int result = JOptionPane.showOptionDialog(null, scrollPane, "Information", JOptionPane.DEFAULT_OPTION,JOptionPane.PLAIN_MESSAGE, null, options, options[0]);
        				switch (result) {	//Tar fram ID för vald kund
        					case 0:
        						...
        						switch (result2) {	//Uppdatering av värden för vald kund
        							case 0:
        							if (cb3.getSelectedItem().toString() == "First Name"){
        								sqlobject.updateCustomerFName(newValue.getText(), Integer.parseInt(wf1.getText()));
        							} else if (cb3.getSelectedItem().toString() == "Last Name") {
        							...
        							updateSingle.setVisible(false);
        							break;
        							case 2:		//Borttagning av vald kund
        							sqlobject.removeCustomer(Integer.parseInt(wf1.getText()));
        							JOptionPane.showMessageDialog(null, "Customer "+Integer.parseInt(wf1.getText())+" removed!", "Information", JOptionPane.INFORMATION_MESSAGE);
        							updateSingle.setVisible(false);
        							break;
        					...
        					case 1:
        					break;
        					case 2:		//Tar fram bokningar för vald kund
        					cb5.removeAllItems();
    ...
    ActionListener bl = new ActionListener() {	 // Listener för huvudmenyn
	    public void actionPerformed(ActionEvent e) {
    ...
    public void arrayListToTableModelSafari (ArrayList<Safari> list, JTable table) { //Metod för att mata in data från en safariarraylist till ett JTable
    ...
    public void arrayListToTableModelCustomer (ArrayList<Customer> list, JTable table) {	//Metod för att mata in data från en kundarraylist till ett JTable
        DefaultTableModel model = new DefaultTableModel();
    ...
    public void arrayListToTableModelBooking (ArrayList<Safaribooking> list, JTable table) {	//Metod för att mata in data från en bokningsarraylist till ett JTable
        DefaultTableModel model = new DefaultTableModel();
    ...
    public static boolean isParsable(String input) { //Kontrollerar så att en String går att omvandla till int. 
